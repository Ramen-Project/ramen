...
export class WebSocketClient {
   ...
   * 發送訊息並等待回應
   */
  async sendRequest<T = any>(type: MessageType, data?: any, timeout: number = 10000): Promise<T> {
   ...
   * 訂閱特定類型的訊息
   */
  subscribe(messageType: MessageType | string, handler: MessageHandler): () => void {
...
/**
 * 初始化全域 WebSocket 客戶端並連接
 */
export async function initializeWebSocketClient(url?: string): Promise<WebSocketClient> {
