...
export class WebSocketManager {
        ...
        switch (message.type) {
            case 'execution_started':
                vscode.window.showInformationMessage(
                    `Execution started: ${message.data.execution_id}`
                );
                break;
    ...
    private queueMessage(message: QueuedMessage): void {
        ...
        if (this.messageQueue.length >= this.maxQueueSize) {
            ...
            this.outputChannel.appendLine(
                `Message queue full, removed ${removeCount} old messages`
            );
        }
    ...
    private flushMessageQueue(): void {
        ...
        const sendBatch = () => {
            ...
            if (this.messageQueue.length > 0 && this.ws && this.ws.readyState === WebSocket.OPEN) {
                ...
                this.outputChannel.appendLine(
                    `Flushed ${sent} messages, ${this.messageQueue.length} remaining`
                );
            }
        };
    ...
    private loadPersistedQueue(): void {
        try {
            ...
            if (queueData) {
                ...
                const now = Date.now();
                const validMessages = persistedQueue.filter((msg) => now - msg.timestamp < 3600000);

                if (validMessages.length > 0) {
                    this.messageQueue.push(...validMessages);
                    this.outputChannel.appendLine(
                        `Loaded ${validMessages.length} persisted messages`
                    );
