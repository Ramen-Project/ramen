...
export class SessionManager implements vscode.Disposable {
     ...
     * Request edit access
     */
    async requestEdit(graphPath: string, userId: string): Promise<EditRequest> {
        const session = await this.getSession(graphPath);
    ...
    private startLockChecker(): void {
        ...
        this.lockCheckTimer = setInterval(async () => {
    ...
    /**
     * Handle workspace shutdown - implements vscode.Disposable
     */
    async dispose(): Promise<void> {
        // Stop lock checker
        if (this.lockCheckTimer) {
            clearInterval(this.lockCheckTimer);
            this.lockCheckTimer = undefined;
        }
